...
`interfaces/` is the cross-domain section as a folder rather than a paper appendix. Each mapping has its own document, its own citations, and its own scope of claim. The seventeen interfaces span: NLS instances (BEC, optical solitons, deep-water waves); baryon acoustic oscillations; Chladni cymatic patterns; gamma-frequency neural entrainment; archaeoacoustic resonance in megalithic chambers; structured state space models in machine learning; cosmological expansion as anti-collapse release; mechanistic-interpretability findings on attention-based systems; the critical-brain literature; coupled phase oscillators (Kuramoto with memory); B-cell affinity maturation in adaptive immunity; the variational dynamics of the Friston free-energy principle and active inference; active matter (flocks, swarms, active gels); self-organized criticality; cardiac dynamics; gene-regulation and circadian rhythms; multi-species ecosystem dynamics. The decision to elevate cross-domain mappings to first-class content rather than supplementary material is the most consequential structural choice in this repository. A reader who reaches `interfaces/` understands immediately that the work is asking to be evaluated across domains, not within any one of them. The seventeen interfaces divide into three evidentiary classes (mathematical equivalence, calibration-dependent structural correspondence, mechanism-shape and convergent-program correspondence) catalogued in [`interfaces/README.md`](interfaces/README.md).
