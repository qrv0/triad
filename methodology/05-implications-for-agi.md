...
3. **Cross-domain interfaces → the structure recurs across substrates.** The nine documented instantiations span optical, atomic, hydrodynamic, plasma, cosmological, biological, archaeoacoustic, and computational substrates, plus the convergent observations from the mechanistic-interpretability program on attention-based systems and from the critical-brain literature on cortical dynamics (see [`../interfaces/`](../interfaces/)). The structural form is the same in each; the substrate-specific calibrations and evidentiary classes differ.
