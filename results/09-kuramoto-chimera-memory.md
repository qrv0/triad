...
A result that does not match a prediction is part of active research, not a setback. Per [`../methodology/02-limits-of-falsification.md`](../methodology/02-limits-of-falsification.md), the global structural claim of the work (the equation's form recurs across substrates) is evaluated by cross-domain coherence across the documented interfaces; inconsistent evidence within specific interfaces shifts evidentiary weight against specific calibrations under criterion 4 without bearing on the global claim. The interface 10 mapping (memory-Kuramoto is mathematically the same Markovian embedding as the equation's auxiliary-field memory) is unaffected. The companion test in [`14-kuramoto-chimera-fdt.md`](14-kuramoto-chimera-fdt.md) explores P10.1 at different parameter values.
