...
- Produces evidence inconsistent with P6.1 (no difference or FDT-locked variant has higher variance): the specific calibration of P6.1 is challenged; status moves to "tested in coupled regime, inconsistent" and the prompt becomes investigation of (a) the calibration of P6.1 in the SimSiam substrate, (b) the auxiliary numerical assumptions (Duhem-Quine), or (c) the implementation. Per methodology/02, this shifts evidentiary weight against the specific P6.1 reading under criterion 4 without falsifying the global structural claim of interface 06 (which rests on the mathematical equivalence of the auxiliary-field equation and the diagonal SSM update at the equation level).
