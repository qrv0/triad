...
The mechanistic-interpretability substrate has no physical time; the relevant timescales are computational. The fast scale is one forward-pass step of the model (one attention computation per layer, one token of sequence advance). The medium scale is the context length the model conditions on, on the order of 4k-200k tokens for contemporary production models, which sets how far back categorical structure must persist through the residual stream. The slow scale is training duration (gradient-step time), on the order of $10^5$-$10^7$ steps, which sets when feature directions stabilize in activation space.

The relevant memory hierarchy for the structural argument is the medium-slow pair: context length (forward-pass memory) and training duration (where the features get encoded). Their ratio governs whether features that vary on slow document-level structure can be learned at all by the architecture; in attention-only architectures the medium-scale memory is bounded by the explicit context window and the slow-scale features are then forced into the residual stream as superposed projections.

Per [`../methodology/07-time-as-calibration.md`](../methodology/07-time-as-calibration.md), the calibration choice for this interface fixes one unit of computational time to one model forward-pass step. Under this calibration the equation's auxiliary-field eigenvalues $\nu_j$ correspond to per-step decay rates of memory channels in a P2-instantiating architecture; the equation's slowest $\nu_j$ corresponds to the longest-range categorical feature the model can hold as state without forcing it through the residual stream. In an attention-only architecture (no P2), the equation's $\nu_j$ structure has no analogue, and the slow categorical structure must go elsewhere, the residual stream is where it ends up.

This calibration is the same as the calibration used in interface 06 (state space models): one forward-pass step. The two interfaces are about the same substrate (sequence-modeling architectures); interface 06 documents the mathematical equivalence between the equation's auxiliary fields and the SSM hidden-state update, and the present interface documents the structural prediction for the architectural family lacking that equivalence (attention-only). The cross-interface consistency on the calibration choice is required by [`../methodology/06-calibration-philosophy.md`](../methodology/06-calibration-philosophy.md): both interfaces refer to the same substrate, so the calibration must agree.
