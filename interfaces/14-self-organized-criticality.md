...
- **Prediction P14.2: SOC and MNSM release-transition produce statistically indistinguishable avalanche distributions at appropriate calibration.** The structural prediction is that the present equation's release transition, when simulated with appropriate parameters (effective drive timescale, dimensional dimensionality), should produce avalanche-size distributions statistically indistinguishable from BTW sandpile distributions at matching substrate parameters.
  ...
  - Status: **pending methodologically valid test (wave 1 retracted)**, see [`../results/13-soc-vs-mnsm-avalanches.md`](../results/13-soc-vs-mnsm-avalanches.md) and its retraction note. The MNSM portion of the wave-1 test was run in the isolated regime ($\gamma_0=0$, $T=0$); the perturbations were ad-hoc rather than FDT-coupled. The "partially tested (consistent at coarse level)" claim is withdrawn. A wave-2 test with FDT-coupled MNSM and matched-substrate avalanche-detection methodology is required.
