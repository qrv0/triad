...
- **Prediction P14.2: SOC and MNSM release-transition produce statistically indistinguishable avalanche distributions at appropriate calibration.** The structural prediction is that the present equation's release transition, when simulated with appropriate parameters (effective drive timescale, dimensional dimensionality), should produce avalanche-size distributions statistically indistinguishable from BTW sandpile distributions at matching substrate parameters.
  ...
  - Status: **partially tested (consistent at coarse level)**, see [`../results/13-soc-vs-mnsm-avalanches.md`](../results/13-soc-vs-mnsm-avalanches.md). Side-by-side comparison of BTW sandpile (2D $64\times64$) and MNSM 2D release regime (with periodic perturbations) gives power-law-fit exponents $\tau_{\text{BTW}} \approx 1.37$ and $\tau_{\text{MNSM}} \approx 1.13$, both in the broadband-criticality range $\tau \in [1.0, 1.5]$. The result is consistent with P14.2 at the mechanism-shape level but limited by sparse MNSM event statistics; the strong "statistical indistinguishability" version requires longer MNSM runs or matched-substrate event definitions, deferred to wave 2.
