...
Under P2, the relevant cross-domain question is "does this system exhibit both instantaneous self-coupling and temporal memory?" The answer in the cases documented in [`../interfaces/`](../interfaces/) is uniformly yes. The clearest mathematical instance is the correspondence with structured state space models (see [`../interfaces/06-state-space-models.md`](../interfaces/06-state-space-models.md)): the hidden state of an SSM is, formally, the auxiliary-field embedding of an integral memory, and the nonlinearity that recent variants such as Mamba introduce through input-dependent gating is, formally, an approximation to the cubic self-interaction P2 demands. The two parts of P2 are present in both formulations; they are present in the architecture because the architecture is solving the same structural problem as the equation.
