...
Two predictions internal to interface 06 were tested numerically in the present work (P3 active throughout per the methodology of Section 7). For P6.1 (FDT-locked noise reduces training trajectory variance), a two-variant comparison across $\gamma_0 \in \{0.005, 0.02\}$ at fixed $T = 0.01$, 8000 steps on TinyShakespeare with 1.5M parameters on an RTX 4060, yields a val-loss trajectory standard deviation of 0.0952 at $\gamma_0 = 0.02$ versus 0.0995 at $\gamma_0 = 0.005$, matching the predicted direction (variance decreases as bath coupling grows in the coupled regime). Zero loss spikes in both variants. For P6.3 (cubic state nonlinearity prevents SimSiam collapse in the coupled regime), a cubic-versus-linear SimSiam-without-stop-gradient comparison with FDT noise active, 4000 steps, 95k parameters, yields a final effective rank of 4.60 out of 64 for the cubic variant against 2.88 out of 64 for the linear baseline, a 60% relative preservation of representational rank attributable to the cubic state nonlinearity. Both results are documented in `results/16-fdt-locked-noise-empirical-p3.md` and `results/17-cubic-ssm-simsiam-fdt.md` respectively.
