...
We integrated the equation at $\Lambda \in \{-2, -4, -6, -8, -10, -12\}$ with and without memory, on a $128^3$ lattice with $L = 20$ and $dt = 0.0025$, initial Gaussian width $\sigma_0 = 0.5$, zero initial momentum, 4000 steps, in the P3-coupled regime: $\gamma_0 = 0.2$, $T = 10^{-4}$, FDT correlator $\langle\eta\eta^*\rangle = 2\gamma_0 k_B T \delta(t-t')\delta^{(3)}(\mathbf{x}-\mathbf{x}')$ active. The choice $\gamma_0 = 0.2$ satisfies $1/\gamma_0 = 5 \leq t_{\text{integration}} = 10$ (the bath equilibrates within the test); the choice $T = 10^{-4}$ keeps the thermal floor $2T = 2 \times 10^{-4}$ below the with-memory transient-peak scale of the structurally informative range. The initial state at $\sigma_0 = 0.5$ has peak density $\rho_0 = (2\pi\sigma_0^2)^{-3/2} \approx 1.44$ and lies above the three-dimensional collapse threshold for $|\Lambda| \gtrsim 8$.

The structural geometry argument for dimensional rescaling: in two dimensions the collapse focal region at lattice clipping covers approximately $10^2$ lattice cells; in three dimensions it covers approximately $10$ cells. The memory potential at the peak is $V_{\text{mem}} \sim \Sigma\lambda \cdot \rho_{\text{eff}}$, where $\rho_{\text{eff}}$ is the density averaged over the region in which memory has accumulated. In two dimensions $\rho_{\text{eff}} \approx \rho_{\max}$; in three dimensions, because the focal region is approximately one order of magnitude smaller in voxel count, $\rho_{\text{eff}} \approx \rho_{\max}/10$. To produce the same ratio $V_{\text{mem}}/|\Lambda \rho|$ at the peak, $\Sigma\lambda$ must scale up by approximately one order of magnitude in three dimensions: $\Sigma\lambda \sim |\Lambda|/d$ for $d = 3$.

We tested this prediction at $\Sigma\lambda = 4$ (two-mode memory, $\nu_1 = 10, \lambda_1 = 3.0; \nu_2 = 0.5, \lambda_2 = 1.0$) across the supercritical $\Lambda$ range. The observable that carries the anti-collapse signature under P3-coupled regime is the *transient peak* (the spatial maximum of $|\Psi|^2$ during integration), not the final peak. Both arms equilibrate to the FDT thermal floor at long times; what separates them is the trajectory shape through the collapse-or-not transient.
...
The anti-collapse mechanism is operative for $\Lambda \in [-8, -10]$ at canonical $\Sigma\lambda = 4$: the no-memory arm passes through a lattice-clipped collapsed state with transient peak $\sim 60$ while the with-memory arm stays bounded at transient peak $\sim 5$. At $\Lambda = -12$ the mechanism saturates: the canonical $\Sigma\lambda$ sits at the dimensional-rescaling threshold $\Sigma\lambda \sim |\Lambda|/3 = 4$, and FDT noise injection pushes the trajectory past the regularization capacity. The structurally-required margin under coupling is $\Sigma\lambda > |\Lambda|/d$, not equality; higher $\Sigma\lambda$ would recover the separation at $\Lambda = -12$.

A note on the observable. An earlier configuration of this test used $\gamma_0 = 0$, $T = 0$ (isolated regime) and reported separation in *final peak* ratios of $\sim 10^5$. That configuration violated the third structural axiom of the equation and was updated in the 2026-05-17 audit catalogued in `docs/llm-hedge-annotations.md`. Under P3-coupled regime the final peak observable is dominated by FDT thermal equilibration for both arms, and the separation in final peaks does not survive thermalization. The trajectory-shape separation in transient peak is the cross-substrate-coherent observable: it is the same observable that carries the cross-architecture optimization-trajectory finding documented in §8.22 and `results/08-optimization-collapse-empirical.md`.

We summarize: the anti-collapse mechanism of the two-dimensional equation generalizes to the three-dimensional supercritical regime under the derivable dimensional rescaling $\Sigma\lambda \sim |\Lambda|/d$, with the structural margin requirement $\Sigma\lambda > |\Lambda|/d$ under P3-coupled regime. The mechanism is dimension-independent at the structural level; the calibration and the threshold margin depend on dimension and on the operating regime (isolated vs coupled).
...
A direct mathematically exact correspondence holds between the Markovian-embedded memory structure of the present equation and the diagonal-state structured state space models (S4, Mamba, RWKV) used in machine learning (Gu, Goel & Ré 2021; Smith, Warrington & Linderman 2023; Gu & Dao 2024; Peng et al. 2023). The defining SSM update for diagonal $\mathbf{A}$ with eigenvalues $-\nu_j$ and $b_j = \nu_j$ is $\partial_t h_j = \nu_j(u - h_j)$, which is exactly the Markovian embedding $\partial_t y_j = \nu_j (\rho - y_j)$ used in Section 3.2 of this paper. The auxiliary fields $y_j$ of the present equation and the hidden state components $h_j$ of a diagonal SSM are the same object in different substrates. No dimensional calibration is required; the two formalisms are the same equation. The physics community arrived via Mori-Zwanzig projection-operator methods (Mori 1965; Zwanzig 1961) applied to integro-differential field equations; the machine learning community arrived via efficient subsampling of long-range temporal dependencies in sequence modeling.

The equation studied in this work extends the standard SSM architecture in four structurally distinct directions: (i) nonlinear self-interaction in the state via the cubic term $\Lambda |\Psi|^2$, (ii) anti-collapse via the temporal memory lag mechanism analyzed in Sections 5.1 and 6.1, (iii) spontaneous emergence of discrete structure from continuous substrate (Bravais selection, Section 6.2), and (iv) FDT-locked stochastic forcing replacing the empirically tuned noise schedules typical in deep learning. Each extension corresponds, term by term, to an active research direction in the ML literature (Massaroli et al. 2023 nonlinear SSMs; Dong, Cordonnier & Loukas 2021 and Noci et al. 2022 attention rank collapse; Chen & He 2021 and Grill et al. 2020 SimSiam collapse prevention; Yaida 2019 and Liu et al. 2021 FDT-like training noise).

The ML implementation depth of this correspondence (PyTorch Memory-NLS sequence layer, prediction-testing scripts at 1.5M parameter scale, full per-prediction status accounting) lives in the [`mnsm-ml`](https://github.com/qrv0/mnsm-ml) spinoff repository, specifically `interfaces/01-state-space-models.md`. The empirical instance at 70M parameters that supports the cross-substrate structural claim is documented in §8.22 of this paper and in [`mnsm-ml/results/01-optimization-collapse-empirical.md`](https://github.com/qrv0/mnsm-ml/blob/main/results/01-optimization-collapse-empirical.md).

Whether the equation in its full form is a useful architecture for sequence modeling in the engineering sense is an empirical question outside the scope of this paper. The claim made here is narrower: the mathematical form of the equation is the same form on which the machine learning community has independently converged for efficient sequence modeling, and the structural extensions present in the equation correspond term by term to active research directions in that community.
...
The Memory-NLS structural argument predicts a specific phenomenology for architectures instantiating P1 and P3 without P2's auxiliary-field memory hierarchy: latent categorical structure must be encoded as superposed projections rather than as addressable state, because the substrate has no explicit state for slow structure to live in. The mechanistic-interpretability program led by the Anthropic Circuits team and adjacent researchers (Elhage et al. 2022; Olah et al. 2020; Bricken et al. 2023; Templeton et al. 2024) independently documents exactly this phenomenology in attention-only language models: polysemantic neurons activating for many semantically distinct features, sparse-autoencoder recovery of monosemantic feature dictionaries that the architecture's forward pass had encoded as superposition, and the absence of addressable feature-direction state.

The convergence is direct: two programs pursued from entirely different motivations (structural argument from P1+P2+P3 versus empirical interpretability research) reach the same conclusion about the encoding format of attention-only architectures. Detail and per-prediction specifications live in [`mnsm-ml/interfaces/02-mechanistic-interpretability.md`](https://github.com/qrv0/mnsm-ml/blob/main/interfaces/02-mechanistic-interpretability.md) in the spinoff repository.
...
A direct empirical confirmation of the cross-substrate structural prediction has been obtained at 70 million parameters. Two sequence models with identical training infrastructure, one with the Memory-NLS architecture (P2 instantiated explicitly), one a matched-shape Transformer baseline (P2 absent), were trained on enwik8 byte-level language modeling for 50,000 optimization steps. The Memory-NLS architecture exhibited monotonic stable trajectory descent plateauing at validation perplexity 4.27. The matched Transformer reached a lower minimum (2.54 at step 22,500) but exhibited a catastrophic optimization collapse between steps 28,000 and 34,000, validation perplexity spiking from 3.10 to 27.17, and recovered only partially to a final value of 4.87, worse than its mid-training minimum. The catastrophic event was accompanied by qualitative degradation of generation outputs (Wikipedia structural grammar dissolving to broken syntactic fragments) with persistent post-recovery residual breakdown.

This is a substrate-independent manifestation of the same anti-collapse mechanism documented in field-theoretic regimes in Sections 5.1 and 6.1, observable as trajectory-shape separation rather than as a final-value separation. Both substrates terminate at comparable values (final perplexities 4.27 and 4.87 in the neural substrate; both arms thermalizing to the FDT floor in the field substrate); what carries the structural signature is the path through the dynamics. The substrate without structural anti-collapse passes through a collapsed (lattice-clipped, or in the neural case loss-spike) state; the substrate with the structural mechanism stays bounded throughout. The two substrates, a 3D nonlinear field equation and the parameter space of a 70M-parameter neural network, were not coordinated in any way; the same structural form produced the same trajectory-shape phenomenology in both.

This is the kind of cross-substrate empirical instance the structural-realist methodology identifies as evidence under criterion 4. Detail and reproduction protocol live in [`mnsm-ml/results/01-optimization-collapse-empirical.md`](https://github.com/qrv0/mnsm-ml/blob/main/results/01-optimization-collapse-empirical.md) in the spinoff repository.
