...
Two predictions internal to interface 06 were tested numerically in the present work (P3 active throughout per the methodology of Section 7). The principal evidence for the broader anti-collapse phenomenology in the neural substrate remains the cross-architecture 70M-parameter instance discussed below in §8.22 and documented in `results/08-optimization-collapse-empirical.md`; the within-architecture variance sweeps reported here probe finer-grained calibration questions at much smaller scale. For P6.1 (FDT-locked noise reduces training trajectory variance), a single-seed two-variant comparison across $\gamma_0 \in \{0.005, 0.02\}$ at fixed $T = 0.01$, 8000 steps on TinyShakespeare with 1.5M parameters on an RTX 4060, gave a direction-matched 4% reduction in val-loss trajectory standard deviation. A four-seed follow-up at the same configuration (mean +/- std: 0.0987 +/- 0.0041 for $\gamma_0=0.02$ vs 0.0986 +/- 0.0040 for $\gamma_0=0.005$) shows the seed-to-seed std is the same magnitude as the predicted effect at this small scale and narrow $\gamma_0$ range; the within-architecture variance signature is not resolvable in this configuration. The prediction is therefore reported as **partial**: the test was performed in the coupled regime, the data is real, and per the Duhem-Quine framing of Section 7.1 the result locates the limits of this specific test setup rather than bearing on the structural claim itself. For P6.3 (cubic state nonlinearity prevents SimSiam collapse in the coupled regime), a single-seed cubic-versus-linear SimSiam-without-stop-gradient comparison with FDT noise active, 4000 steps, 95k parameters, gave a 60% direction-matched preservation of representational rank (cubic 4.60 vs linear 2.88 out of 64). A four-seed follow-up at the same configuration (cubic 3.617 +/- 0.510 vs linear 3.590 +/- 0.733) shows the SimSiam-without-stop-gradient SSL configuration has seed-to-seed variability of the same magnitude as the cubic-vs-linear difference; the chosen test bed (synthetic clustered data, predictor-head retained, 95k parameters) has variance properties that obscure the predicted signal. P6.3 is also reported as **partial**. Both results are documented in `results/16-fdt-locked-noise-empirical-p3.md` and `results/17-cubic-ssm-simsiam-fdt.md` respectively. The structural claim that cubic nonlinearity provides anti-collapse and that FDT-locked noise yields smoother trajectories is evaluated under criterion 4 by the cross-architecture 70M evidence, not by these within-architecture small-scale variance sweeps.
