...
We map the equation's structural form onto seven independently documented domains plus one cross-substrate empirical confirmation in neural training dynamics. In each case the mapping is mathematical, same form, different physical instantiation, and the citation is peer-reviewed. The argument advanced is not that the equation predicts these phenomena under a privileged dimensional identification; the argument is that the same structural form appears at multiple scales of physical organization, and that this constitutes the principal cross-domain coherence test that a structural realist applies.
...
### 8.8 Cross-substrate empirical confirmation in neural training dynamics
