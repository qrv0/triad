...
**Prior wording.** "MNSM will outperform attention-based
...
**Replacement wording.** "MNSM instantiates a different
...
3. `test_fdt_locked_noise_multiseed.py` (Phase L): wrapper script that imports configuration from Phase C canonical `experiments/neural/test_fdt_locked_noise.py` (the neural experiments later migrated to the [`mnsm`](https://github.com/qrv0/mnsm) spinoff). No independent configuration to audit; inherits canonical setup. Multi-seed result data was documented in `results/16-fdt-locked-noise-empirical-p3.md` as partial; this result document was never finalized and is part of the historical audit record only.
4. `test_simsiam_cubic_ssm_multiseed.py` (Phase L): wrapper script that imports configuration from Phase C canonical `experiments/neural/test_simsiam_cubic_ssm.py` (also migrated to the [`mnsm`](https://github.com/qrv0/mnsm) spinoff). No independent configuration to audit. Multi-seed result documented in `results/17-cubic-ssm-simsiam-fdt.md` as partial; this result was likewise never finalized.
...
(moved to [`mnsm/results/01`](https://github.com/qrv0/mnsm/blob/main/results/01-optimization-collapse-empirical.md)
