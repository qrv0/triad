"""Phase diagram 2D slice at d=3 using Convention A (dimensional-rescaling series).

Convention A: sigma_init=0.4 with non-normalized Gaussian formula. See
test_phase_diagram_d2_convention_A.py for the convention rationale.

Companion to test_phase_diagram_2d_slice.py which uses Convention B
(sigma=0.5 normalized).

Output: outputs/phase_diagram_d3_convention_A/summary.json.
Wall time estimate: ~5-8 min on RTX 4060 (N=48, 20 grid points, 2000 steps).
"""
...
OUTPUT_DIR = REPO_ROOT / "outputs" / "phase_diagram_d3_convention_A"
...
def initial_gaussian_convention_A_3d(N: int, L: float, sigma_init: float):
    """Convention A: 3D Gaussian, NON-normalized."""
    coord = xp.linspace(-L / 2, L / 2, N, endpoint=False)
    X, Y, Z = xp.meshgrid(coord, coord, coord, indexing="ij")
    r_squared = X ** 2 + Y ** 2 + Z ** 2
    prefactor = (1.0 / (sigma_init * float(np.sqrt(2 * np.pi)))) ** (3 / 2)
...
def run_grid_point(N: int, L: float, Lambda: float, Sigma_lambda: float,
    ...
    psi = initial_gaussian_convention_A_3d(N, L, sigma_init)
...
def main():
    print(f"Phase diagram d=3 Convention A (sigma=0.4 non-normalized)")
    ...
    dx = L / N
    print(f"\nInitial state (d=3, sigma=0.4, Convention A):")
    print(f"  Peak |Psi|^2: {float(xp.max(xp.abs(psi_init) ** 2)):.4f}")
    print(f"  Discrete L2 norm: {float(xp.sum(xp.abs(psi_init) ** 2) * dx ** 3):.4f}")
    print(f"\nConfiguration:")
    ...
    summary = {
        "prediction_tested": "Phase diagram d=3 slice, Convention A (sigma=0.4 non-normalized)",
        "convention": "A",
        "config": {
            ...
            "initial_norm": float(xp.sum(xp.abs(psi_init) ** 2) * dx ** 3),
        },
